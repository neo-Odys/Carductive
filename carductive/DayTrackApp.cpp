...
const int WORK_CATEGORIES[] = {2, 17, 18, 19};
const int WORK_CATEGORIES_COUNT = 4;
...
void DayTrackApp::drawTimeline()
{
  ...
  for (int i = 0; i < 24; i++)
  {
    int id = daySchedule[i];
    for (int j = 0; j < WORK_CATEGORIES_COUNT; j++)
    {
      if (id == WORK_CATEGORIES[j])
      {
        productiveHours++;
        break;
      }
  ...
  int freeMinutes = productiveHours * 15;

  canvas.setTextSize(1);
  canvas.setTextDatum(bottom_left);
  canvas.setTextColor(CAT_STUDY);
  char workStr[20];
  snprintf(workStr, sizeof(workStr), "%dh work", productiveHours);
  canvas.drawString(workStr, 5, 130);

  canvas.setTextDatum(bottom_right);
  canvas.setTextColor(CAT_GAMES);
  char freeStr[20];
  if (freeMinutes >= 60)
  {
    snprintf(freeStr, sizeof(freeStr), "%dh%dmin free", freeMinutes / 60, freeMinutes % 60);
  }
  else
  {
    snprintf(freeStr, sizeof(freeStr), "%dmin free", freeMinutes);
  }
  canvas.drawString(freeStr, 235, 130);
...
void DayTrackApp::drawLegendScreen()
{
  ...
  canvas.drawString("6: READ/GAM/RLX", 125, y); // <-- Zaktualizowana nazwa w legendzie!
  y += 12;
